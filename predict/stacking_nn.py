...
for fold, (train_idx, valid_idx) in enumerate(kfold.split(train_X, train_y)):
    ...
    #           callbacks=callbacks)
    model = load_model(f"../models/nn_stacking/{fold}.h5", custom_objects={"f1": f1})
    pred += model.predict(test_X.values) / N_FOLDS
    oof[valid_idx, :] = model.predict(X_valid)
columns = ["nn_0", "nn_1", "nn_2", "nn_3"]

pd.DataFrame(pred, columns=columns).to_csv(f"../for_train_data/nn_stacking/test_nn.csv", index=False)
pd.DataFrame(oof, columns=columns).to_csv(f"../for_train_data/nn_stacking/train_nn.csv", index=False)
print("DONE")
